...
function normaliseOffset(input, mode, customOffset, timezoneOffset, locationOffset) {

  function locationToTimezone(o) {
    if (o !== undefined && o.includes(',')) {
      o = o.split(',');
      return geo.tz(o[0].trim(), o[1].trim());
    }
    return false;
  }

  function timezoneToOffset(o) {
    if (o !== undefined && o.includes('/')) {
      return tz.zone(o).utcOffset(utc(input, 0)) * -1;
    }
    return false;
  }
  ...
  switch (mode) {
    case 'CUSTOM':      
      return timezoneToOffset(locationToTimezone(customOffset) || customOffset) || customOffset;
    case 'LOCATION':
      return timezoneToOffset(locationToTimezone(locationOffset));
    case 'ZONE':
      return timezoneToOffset(timezoneOffset);
    case 'NONE':
    default:
      return false;
  }
...
module.exports = {
  ...
  encode: encode,
  now: moment.utc,
  normaliseOffset: normaliseOffset
}
